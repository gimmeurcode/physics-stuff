...
An interactive mathematics and physics laboratory: 40 wings, 601 guided experiments,
...
2. **Module load order is the filename order.** `build.ps1` concatenates
   ...
   `60b-` loads after `60a-` and before `61-`. All 232 modules share one global
...
3. **Verify before claiming done.** Run `./smoke.ps1` first — it takes ten
   ...
   (4337 unit tests), and `./runstagetests.ps1` must print `0 failed` — it calls
   ...
   **For anything touching `mkPlot`, `plotCurve` or the viewport, also
   ...
   the pan/zoom layer works on all 179 stages *and* that with no interaction
   ...
   **For anything that draws, also `./auditsize.ps1` (`findings=0`) and
   ...
   nothing. `auditsize` sweeps eight canvas shapes across all 179 stages;
