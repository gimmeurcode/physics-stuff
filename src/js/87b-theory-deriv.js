...
${stThm('Implicit function theorem, and the slope it licenses', {
  hyp:'<span class="mth"><i>F</i></span> is continuously differentiable near <span class="mth">(<i>a</i>,<i>b</i>)</span>, <span class="mth"><i>F</i>(<i>a</i>,<i>b</i>) = 0</span>, and <span class="mth"><i>F</i><sub><i>y</i></sub>(<i>a</i>,<i>b</i>) ≠ 0</span>',
  then:'there is an interval around <span class="mth"><i>a</i></span> on which the relation defines a unique differentiable <span class="mth"><i>y</i>(<i>x</i>)</span> with <span class="mth"><i>y</i>(<i>a</i>) = <i>b</i></span>, and',
  eq:'<span class="frac"><span class="nm"><i>dy</i></span><span class="den"><i>dx</i></span></span> <span class="op">=</span> <span class="op">−</span><span class="frac"><span class="nm"><i>F</i><sub><i>x</i></sub></span><span class="den"><i>F</i><sub><i>y</i></sub></span></span>',
  proof:`<p>The formula is the easy half, and it is worth separating the two. <em>Granted</em> that a differentiable <span class="mth"><i>y</i>(<i>x</i>)</span> exists with <span class="mth"><i>F</i>(<i>x</i>, <i>y</i>(<i>x</i>)) = 0</span> identically, differentiate that identity by the chain rule:</p>
${stEq('<i>F</i><sub><i>x</i></sub> <span class="op">+</span> <i>F</i><sub><i>y</i></sub> <span class="op">·</span> <span class="frac"><span class="nm"><i>dy</i></span><span class="den"><i>dx</i></span></span> <span class="op">=</span> 0')}
<p>and solve, which is legal exactly because <span class="mth"><i>F</i><sub><i>y</i></sub> ≠ 0</span>. Note what was <em>not</em> needed: any formula for <span class="mth"><i>y</i></span>.</p>
<p>The hard half is the existence granted above, and it is where the hypothesis earns its place. Suppose <span class="mth"><i>F</i><sub><i>y</i></sub>(<i>a</i>,<i>b</i>) &gt; 0</span>. By continuity of <span class="mth"><i>F</i><sub><i>y</i></sub></span> there is a rectangle around <span class="mth">(<i>a</i>,<i>b</i>)</span> on which <span class="mth"><i>F</i><sub><i>y</i></sub> &gt; 0</span>, so on each vertical slice of it <span class="mth"><i>F</i></span> is <em>strictly increasing</em> in <span class="mth"><i>y</i></span>. Then <span class="mth"><i>F</i>(<i>a</i>, <i>b</i>−δ) &lt; 0 &lt; <i>F</i>(<i>a</i>, <i>b</i>+δ)</span> for small <span class="mth">δ</span>, and by continuity in <span class="mth"><i>x</i></span> those two strict inequalities survive for every <span class="mth"><i>x</i></span> near <span class="mth"><i>a</i></span>. The intermediate value theorem then supplies a root on that slice, and strict monotonicity makes it the <em>only</em> one — which is what defines <span class="mth"><i>y</i>(<i>x</i>)</span>. Differentiability follows from the mean value theorem applied to the same slices.</p>
<p>Every step used <span class="mth"><i>F</i><sub><i>y</i></sub> ≠ 0</span>, and the conclusion genuinely fails without it: the lemniscate <span class="mth">(<i>x</i>²+<i>y</i>²)² = 4(<i>x</i>²−<i>y</i>²)</span> has <span class="mth">∇<i>F</i> = 0</span> at the origin, where it crosses itself and no single-valued <span class="mth"><i>y</i>(<i>x</i>)</span> exists on any interval, however small.</p>`,
  note:'The proof is also an algorithm, and the laboratory runs it: the branch through your x is found by bisection on F over a vertical slice — the intermediate-value step, made numerical — and the slope is then measured a second time as a difference quotient of that branch, with no derivative taken anywhere. Agreement to the secant\'s own h² is the evidence that the rule is the chain rule and not a mnemonic.',
  see:'deriv:clImplicit', seeLabel:'Differentiate a curve nobody can solve for y' })}

${stThm('Derivative of an inverse function', {
  hyp:'<span class="mth"><i>f</i></span> is differentiable and strictly monotone near <span class="mth"><i>a</i></span>, with <span class="mth"><i>f</i>′(<i>a</i>) ≠ 0</span>, and <span class="mth"><i>b</i> = <i>f</i>(<i>a</i>)</span>',
  then:'<span class="mth"><i>f</i><sup>−1</sup></span> is differentiable at <span class="mth"><i>b</i></span> and',
  eq:'(<i>f</i><sup>−1</sup>)′(<i>b</i>) <span class="op">=</span> <span class="frac"><span class="nm">1</span><span class="den"><i>f</i>′(<i>a</i>)</span></span>',
  proof:`<p>This is the implicit function theorem applied to <span class="mth"><i>F</i>(<i>x</i>,<i>y</i>) = <i>f</i>(<i>y</i>) − <i>x</i></span>, and it is worth seeing that it is not a second idea. Here <span class="mth"><i>F</i><sub><i>x</i></sub> = −1</span> and <span class="mth"><i>F</i><sub><i>y</i></sub> = <i>f</i>′(<i>y</i>)</span>, so the theorem's own formula gives <span class="mth"><i>dy</i>/<i>dx</i> = 1/<i>f</i>′(<i>y</i>)</span> — with the derivative of <span class="mth"><i>f</i></span> evaluated at <span class="mth"><i>y</i> = <i>a</i></span>, not at <span class="mth"><i>x</i> = <i>b</i></span>.</p>
<p>Directly: monotonicity makes <span class="mth"><i>f</i><sup>−1</sup></span> exist and be continuous, so <span class="mth"><i>k</i> = <i>f</i><sup>−1</sup>(<i>b</i>+<i>h</i>) − <i>a</i> → 0</span> as <span class="mth"><i>h</i> → 0</span>, and <span class="mth"><i>k</i> ≠ 0</span> for <span class="mth"><i>h</i> ≠ 0</span> because <span class="mth"><i>f</i></span> is injective — which is what makes the following division legal, the same obstacle the chain rule had to route around:</p>
${stEq('<span class="frac"><span class="nm"><i>f</i><sup>−1</sup>(<i>b</i>+<i>h</i>) − <i>f</i><sup>−1</sup>(<i>b</i>)</span><span class="den"><i>h</i></span></span> <span class="op">=</span> <span class="frac"><span class="nm"><i>k</i></span><span class="den"><i>f</i>(<i>a</i>+<i>k</i>) − <i>f</i>(<i>a</i>)</span></span>')}
<p>The right-hand side is the reciprocal of a difference quotient for <span class="mth"><i>f</i></span> at <span class="mth"><i>a</i></span>, which tends to <span class="mth"><i>f</i>′(<i>a</i>) ≠ 0</span>; so the limit exists and is <span class="mth">1/<i>f</i>′(<i>a</i>)</span>.</p>`,
  note:'Where the reciprocal is evaluated is the whole content, and the commonest error in the topic is writing 1/f′(b). The picture settles it: the graph of f⁻¹ is the graph of f reflected in y = x, so the tangent line you are comparing touches at the reflected point (b, a), not at (b, f(b)). Every standard inverse derivative is this rule plus an identity — 1/cos a rewritten as 1/√(1−b²), 1/sec²a as 1/(1+b²), 1/eᵃ as 1/b.',
  see:'deriv:clImplicit', seeLabel:'The derivative of a function you never inverted' })}
