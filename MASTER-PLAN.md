...
| `auditticks.ps1` | ~1 min | `auditticks OK` | what the axis furniture and headings **actually paint** — it wraps `fillText` on the canvas prototype over every stage's live frame. Two checks nothing else can make: **duplicate tick labels** (fmtNum's decimals clamp collapsed any axis with span ≲ 0.01 into `0.002, 0.002, 0.002 …` — the statmech density axis, found only by a screenshot on 2026-08-15; `fmtTick` derives precision from the step and both owners use it), and **canvas text under any of the three DOM overlay boxes** — the chip (top-left), the **legend** (bottom-left) and the **perf strip** (top-right). The chip half existed from the start (`ctTitleClearChip` slides plotFrame/ctFrame titles clear); the legend and perf halves were added 2026-08-19 after a by-eye screenshot sweep found ten stages' `stageNote` captions starting under the legend and three headings under the perf strip — `ctLegendZone`/`ctPerfZone` (60a) are the dodges, and `stageNote`, `plotFrame`'s x- and y-labels and the title path all consult them. **It runs at 1280×900, not 1680×1000**, because the under-legend class only manifests on a canvas narrow enough that a centred caption reaches the legend box. Carries controls in every run: the real `ctGrid` on a small-span window must be clean, the old `fmtNum(v,3)` labelling replayed on it must be flagged, and a heading drawn at the centre of **each visible overlay box** must be flagged — a gate never seen to fail is not known to work |
